...
void ConsoleChat::initSql()
{
  ...
  if (!mysql_real_connect(&mysql, "localhost", "root", "root", "chatdb", 0, NULL, 0))
	//if (!mysql_real_connect(&mysql, "localhost", "admin", "!Patriot1", "chatdb", 0, NULL, 0))  // Подключаемся к серверу
  {
...
void ConsoleChat::startLoad()
{
	initSql();
		...
		query ="CREATE TABLE IF NOT EXISTS users(id SERIAL PRIMARY KEY, login VARCHAR(50) \
	 	NOT NULL, hash_password VARCHAR(60) NOT NULL, email VARCHAR(50) NOT NULL, \
	 	 salt VARCHAR(50) NOT NULL, date_registraion DATETIME DEFAULT NOW())";
	...
	querySql(query);
  	   ...
  	   query = "INSERT INTO users(id, login,  hash_password, email, salt, date_registraion) \
  	   VALUES(0, Всем, 0, 0, 0, DEFAULT)";
  	   querySql(query);
 	start();
}
...
{
	std::string login, password, salt, hash_password ;
...
void ConsoleChat::signUp()//регистрация
{
	...
	try
		{
			if (getUserByLogin(login) || login == "0")
				{
	...
	catch (const LoginException& ex)
		{
		 ...
		 std::string salt = genSalt(); //генератор соли
...
{
	std::string action;
	action = "Добро пожаловать " + _onlineUser->getLogin() + "\n";
	server.insock(action);
